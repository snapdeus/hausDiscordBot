...
test("Test ZapierNLAWrapper", async () => {
    ...
    let result = await zapier.runAction(action?.id ?? "", "Find an email with Silicon Valley Bank");
    console.log("result: ", result);
    result = await zapier.previewAction(action?.id ?? "", "Find an email with foobarbaz");
    console.log("result: ", result);
});
